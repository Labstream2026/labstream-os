...
// - /api/ops/file: archivo vivo de Operaciones_LAB; el Document Server de OnlyOffice lo
//   descarga con token firmado «opsdoc» (sin cookie). La ruta exige sesión O token.
const PUBLIC_PREFIXES = ["/login", "/api/auth", "/review", "/portadas", "/cotizacion", "/p", "/invitacion", "/subir", "/api/proposal-img", "/api/cron", "/api/review-media", "/api/files-asset", "/api/ops/file", "/api/upload", "/api/whatsapp", "/api/openclaw", "/api/v1", "/api/mcp", "/api/calendar/feed", "/api/resolve-plugin", "/api/brand-logo", "/api/health"];

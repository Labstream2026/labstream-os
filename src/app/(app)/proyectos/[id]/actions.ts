...
// Crea una tarea desde UN renglón («Grabar dron mañana 9am @Zahid #rodaje 2h»). El núcleo
// vive en lib/task-quick (compartido con /api/v1/tasks/quick y las herramientas del agente);
// aquí solo se valida el acceso al proyecto y se revalida la UI.
...
// título con el texto, descripción con autor + timecode + enlace a la sala, atada al entregable
// (isDeliverableWork) y asignada al dueño del entregable. La UI la cablea internal-review con
// un botón por comentario; también sirve para el portal del equipo.
export async function createTaskFromReviewComment(commentId: string): Promise<{ ok: boolean; error?: string; taskId?: string }> {
  const session = await getSession();
  if (!session) return { ok: false, error: "No autorizado." };
  const c = await db.reviewComment.findUnique({
    where: { id: commentId },
    select: {
      body: true, timecode: true, authorName: true, fromClient: true,
      deliverable: { select: { id: true, name: true, ownerId: true, projectId: true, project: { select: accessSelect } } },
    },
  });
  if (!c?.deliverable?.projectId || !c.deliverable.project) return { ok: false, error: "El comentario no existe." };
  if (!canWriteProject(c.deliverable.project, session)) return { ok: false, error: "No autorizado." };
  const tc = c.timecode != null
    ? `${Math.floor(c.timecode / 3600) ? `${Math.floor(c.timecode / 3600)}:` : ""}${String(Math.floor((c.timecode % 3600) / 60)).padStart(2, "0")}:${String(Math.floor(c.timecode % 60)).padStart(2, "0")}`
    : null;
  const shortBody = c.body.trim().slice(0, 70) + (c.body.trim().length > 70 ? "…" : "");
  const projectId = c.deliverable.projectId;
  const position = await db.task.count({ where: { projectId } });
  const assigneeId = c.deliverable.ownerId ?? session.id;
  const task = await db.task.create({
    data: {
      title: `Corrección: ${shortBody}`,
      description: `${c.fromClient ? "Comentario del CLIENTE" : "Comentario interno"} de ${c.authorName} en «${c.deliverable.name}»${tc ? ` (${tc})` : ""}:\n\n${c.body.trim()}\n\nSala de revisión: /revisiones/${c.deliverable.id}`,
      projectId,
      deliverableId: c.deliverable.id,
  ...
  if (assigneeId !== session.id) {
    await notifyAndEmail(assigneeId, {
      type: "task",
      event: "task_assigned",
      title: `Corrección en «${c.deliverable.name}»`,
      body: `${shortBody}${tc ? ` · ${tc}` : ""}`,
      link: `/revisiones/${c.deliverable.id}`,
      actorId: session.id,
    }).catch(() => null);
  }
  await logActivity({ action: "task.create", summary: `convirtió un comentario de revisión en tarea: «${shortBody}»`, projectId, entityType: "task", entityId: task.id });
  refresh(projectId);

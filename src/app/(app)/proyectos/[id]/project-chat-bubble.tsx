...
// Chat del proyecto como BURBUJA flotante (FAB abajo-derecha), en vez de una pestaña.
...
export function ProjectChatBubble({ projectId, me, isAdmin }: { projectId: string; me: ChatMe; isAdmin: boolean }) {
  ...
  if (open) {
    return (
      <div className="fixed bottom-4 right-4 z-50 flex h-[min(560px,80vh)] w-[min(384px,calc(100vw-2rem))] flex-col overflow-hidden rounded-2xl border border-border bg-background shadow-2xl duration-200 animate-in fade-in slide-in-from-bottom-4">
  ...
  return (
    <button
      ...
      className={cn(
        "group fixed bottom-5 right-5 z-50 grid size-14 place-items-center rounded-full bg-primary text-primary-foreground shadow-lg shadow-primary/30 transition-transform hover:scale-105 active:scale-95",
      )}
    >

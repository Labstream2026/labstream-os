...
// La misma paleta un par de tonos MÁS CLARA (Tailwind 300), para escribir el color de una
// persona SOBRE fondo oscuro: el nombre del autor dentro de la burbuja del chat. Los 500 de
// arriba nacieron para ir sobre blanco y ahí abajo se apagan (un `blue-500` sobre pizarra
// ronda 2,6:1 y no llega a legible).
export const AVATAR_HEX_SOFT: Record<string, string> = {
  red: "#fca5a5",
  orange: "#fdba74",
  amber: "#fcd34d",
  yellow: "#fde047",
...
export function avatarHexSoft(key?: string | null): string {
  return AVATAR_HEX_SOFT[key ?? ""] ?? AVATAR_HEX_SOFT.slate;
}

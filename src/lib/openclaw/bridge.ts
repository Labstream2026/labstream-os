...
function systemPrompt(askerName: string, askerRole: string): string {
  ...
  return [
    ...
    "IMÁGENES y VIDEO (no inventes): para CREAR/GENERAR/DIBUJAR una imagen usa SIEMPRE generar_imagen (la genera y la entrega en el chat al instante). Para CREAR/GENERAR un video usa generar_video: el video TARDA varios minutos, así que la herramienta solo lo ARRANCA — dile al usuario que se lo enviarás aquí en cuanto esté listo y NO afirmes que ya está. NUNCA digas que enviaste, adjuntaste, generaste o que 'ya está disponible en el chat' una imagen, video o archivo si no ejecutaste de verdad la herramienta que lo entrega: generar_imagen (imágenes), generar_video (videos), send_file (archivos que YA existen en un proyecto), send_quote (cotización en PDF). Si una herramienta falla o no la usaste, dilo con claridad y NO afirmes que entregaste nada.",

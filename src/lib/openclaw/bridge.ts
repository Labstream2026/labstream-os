...
function systemPrompt(askerName: string, askerRole: string): string {
  ...
  return [
    ...
    "IMÁGENES y ARCHIVOS (no inventes): para CREAR/GENERAR/DIBUJAR una imagen usa SIEMPRE la herramienta generar_imagen (ella la genera y la entrega en el chat). NO puedes generar audio ni video. NUNCA digas que enviaste, adjuntaste, generaste o que 'ya está disponible en el chat' una imagen o archivo si no ejecutaste de verdad la herramienta que lo entrega: generar_imagen para imágenes nuevas, send_file para archivos que YA existen en un proyecto, send_quote para la cotización en PDF. Si una herramienta falla o no la usaste, dilo con claridad y NO afirmes que entregaste nada.",

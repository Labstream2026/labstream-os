...
function systemPrompt(askerName: string, askerRole: string): string {
  ...
  return [
    ...
    "LÍMITES IMPORTANTES (no los rompas): NO puedes generar, crear ni dibujar imágenes, ni audio, ni video — no tienes ninguna herramienta para eso. Si te piden crear/generar/hacer una imagen (o cualquier archivo que no exista ya), responde con claridad que TODAVÍA no puedes generar imágenes y no inventes que la hiciste. NUNCA digas que enviaste, adjuntaste, generaste o que 'ya está disponible en el chat' un archivo o imagen si no ejecutaste de verdad una herramienta que lo entregue (solo send_file envía archivos que YA existen en un proyecto, y send_quote envía la cotización en PDF). Si no usaste esas herramientas, NO afirmes que enviaste nada.",

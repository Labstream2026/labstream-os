...
// Marca Labstream Studio — usa los logos OFICIALES de public/brand/ (logo.png negro para
// fondo claro, logo-dark.png blanco para fondo oscuro), cambiando solo con el tema.
...
export function Logo({ className, alt = "Labstream Studio" }: { className?: string; alt?: string }) {
  return (
    <span className={cn("inline-flex items-center", className)}>
      ...
      <img src="/brand/logo.png" alt={alt} className="block h-full w-auto dark:hidden" />
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src="/brand/logo-dark.png" alt={alt} className="hidden h-full w-auto dark:block" />
    </span>
  );
}
